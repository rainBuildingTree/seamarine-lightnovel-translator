...
def translate_chunk_for_enhance(html_fragment, language):
    ...
    prompt = (
        f"당신은 라이트노벨 전문 번역가입니다. 아래에 주어진 텍스트는 기존 한국어 초벌 번역본이지만, 일부 외국어가 그대로 남아 있고 번역이 어색합니다.\n\n"
        
        "📝 번역 지침:\n"
        "- **고유명사(인명, 지명, 작품명 등)는 직역하지 말고, 원어 발음을 존중해 음역해 주세요.**\n"
        "- 전체 문장을 자연스럽고 문학적으로 **다시 다듬어 주십시오.**\n"
        "- 다듬을 때 **문장의 원래 의미나 의도를 바꾸지 마십시오.**\n"
        "- **한국어가 아닌 외국어 표현은 문맥에 맞게 정확히 번역해 반영해 주세요.**\n"
        "- **창작하거나 임의로 해석하지 말고**, 원문 문장의 흐름과 논리를 유지하며 부드럽게 표현해 주세요.\n"
        "- **출력 결과에는 오직 자연스러운 한국어만 포함되어야 하며, 외국어는 절대 포함되지 않아야 합니다.**\n"
        "- ** 「 」, 『 』의 문장부호는 절대로 수정하거나 변경하지 마십시오.**\n"
        "- ** 「 」, 『 』의 문장부호가 닫히지 않았어도 」, 』등을 추가하여 닫지 마십시오.**\n"
        "- **외국어 원문을 병기하거나 '(원문)→(번역문)'와 같은 형식으로 표시하지 마십시오.**\n"
        "- 설명, 주석, 마크다운 등의 출력은 절대 하지 마십시오.\n"
        
        "다음 문장을 자연스럽고 문학적으로 다시 번역해 주세요:\n\n" + processed_html
    )
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            response = client.models.generate_content(
