...
class ImageTranslateSettingViewModel(QObject):
    def __init__(self, config_data: ConfigData, runtime_data, app_controller, parent=None):
        ...
        try:
            ...
            self._ai_config: AiModelConfig = config_data.image_translate_model_config
    ...
    def save_data(
        ...
        try:
            ...
            self._config_data.image_translate_model_config = self._ai_config
    ...
    def set_default(self, property: str):
        try:
            if 'name' in property:
                self._llm_model_name = get_default_config().get('image_translate_model_config').get('name')
                self.llmModelNameChanged.emit()
                self.llmModelIndexChanged.emit()
            elif 'prompt' in property:
                self._sys_prompt = get_default_config().get('image_translate_model_config').get('system_prompt')
                self.sysPromptChanged.emit()
            elif 'temp' in property:
                self._temp = get_default_config().get('image_translate_model_config').get('temperature')
                self.tempChanged.emit()
            elif 'top' in property:
                self._top_p = get_default_config().get('image_translate_model_config').get('top_p')
                self.topPChanged.emit()
            elif 'freq' in property:
                self._freq_penalty = get_default_config().get('image_translate_model_config').get('frequency_penalty')
                self.freqPenaltyChanged.emit()
            elif 'use' in property:
                self._use_thinking_budget = get_default_config().get('image_translate_model_config').get('use_thinking_budget')
                self.useThinkingBudgetChanged.emit()
            elif 'thinking' in property:
                self._thinking_budget = get_default_config().get('image_translate_model_config').get('thinking_budget')
            else:
